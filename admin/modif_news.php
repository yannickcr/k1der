...
<BODY onfocus="TraitEvent()" BGCOLOR=White TEXT=Black LINK=Blue VLINK=Blue ALINK=Blue LEFTMARGIN=0 TOPMARGIN=4 MARGINWIDTH=0 MARGINHEIGHT=4>
   ...
   $req = MYSQL_QUERY("SELECT * FROM mynewsinfos WHERE id='$id'");
   $disp = mysql_fetch_array($req);
   ?>
  <FORM METHOD="POST" action="admin/modif_news2.php" name="webmail" onsubmit="return true;">
    <TABLE width="600" 
        border=0 cellPadding=3 cellSpacing=0 class=forumline>
      <TBODY>
        <TR> 
          <TD colspan="3" class=row1><table width="600" border="0" cellspacing="0" cellpadding="0" align="left"> 
      ...
      <tr> <td width="439" height="20" background="images/fond.gif"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><img src="images/courbe1.gif" width="7" height="20" align="absmiddle"><b><font color="#FFFFFF">-=</font></b></font><font color="#FFFFFF" face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>Modification  
      de la News N°<? echo $id; ?></b></font><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b><font color="#FFFFFF">=-</font></b></font></td></tr> 
      <tr> <td colspan="2">&nbsp;</td></tr> </table></TD>
        ...
        <TR> 
          <TD width="125" class=row1><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><SPAN class=gen><B>Auteur 
            :</B></SPAN></font></TD>
          <TD width="306" class=row2><SPAN class=genmed> <font size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
            <?
			echo $disp[signature];
			?>
            <input name="auteur" type="hidden" id="auteur" value="<? echo $disp[signature]; ?>"><input name="id" type="hidden" id="id" value="<? echo $id; ?>"><input name="date" type="hidden" id="date" value="<? echo $disp[date]; ?>"><input name="heure" type="hidden" id="heure" value="<? echo $disp[heure]; ?>">
            </font></SPAN></TD>
          <TD width="306" class=row2><span class=genmed><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Date 
            :</strong> <? echo $disp[date]; ?> <strong>&amp;</strong> <strong>Heure 
            :</strong> <? echo $disp[heure]; ?> </font></span></TD>
        </TR>
        <TR> 
          <TD width="125" class=row1><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><SPAN class=gen><B>Titre 
            :</B></SPAN></font></TD>
          <TD colspan="2" class=row2><SPAN class=gen> 
            <INPUT name=titre class=post 
            style="WIDTH: 450px" tabIndex=2size=45 value="<? echo $disp[titre]; ?>">
            </SPAN></TD>
        </TR>
        <TR> 
          ...
          <TD colspan="2" class=row2><font size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
			<input name="type" type="radio" value="k1d" <? if($disp[url_source] == 'k1d') { echo 'checked'; } ?>>
            K1der 
            <input type="radio" name="type" value="hl" <? if($disp[url_source] == 'hl') { echo 'checked'; } ?>>
            Half-Life
			<input type="radio" name="type" value="cs" <? if($disp[url_source] == 'cs') { echo 'checked'; } ?>>
            Counter-Strike
			<input type="radio" name="type" value="lan" <? if($disp[url_source] == 'lan') { echo 'checked'; } ?>>
            LAN Party			
            <input type="radio" name="type" value="admin" <? if($disp[url_source] == 'admin') { echo 'checked'; } ?>><font color="red"><b>
            Admin</b></font>		</font></TD>
        </TR>
      </TBODY>
    </TABLE>
...
function default_doit(newName,newValue,newFolder,doPub) {
    ...
    <input type="hidden" name="chTpl" value="">
        <!-- Debut main -->
        <input name="FormatHTML" type="hidden" id="FormatHTML" value="1"> 
        <table width="600" border="0" cellspacing="0" cellpadding="0">
            <tr> 
              <td><table width="600" border="0" cellspacing="0" cellpadding="10" bgcolor="FFFFFF">
                  <tr> 
                  ...
                  <tr> 
                    <td><div id='boite' name='boite' style='height:310px; position: relative; top: 0px; width: 100%; border: 1px solid #000000'> 
                        <textarea wrap="HARD" name="Text2" id="textarea5" rows=1 style='left : 0px; top : 0px ;HEIGHT: 100%; visibility: visible; POSITION: absolute ; WIDTH: 100%; Z-INDEX: 100' cols='20'><? echo $disp[news]; ?></Textarea>
                  ...
                  <tr> 
                    <td width='110'> <input name='Val' type='hidden'> <input type='hidden' name='TypeVal'> 
                      <input type='hidden' name='TypeURL'> <input type='hidden' name='Txt1' value=''> 
                      <input type='hidden' name='Txt2' value=''> <input type='hidden' name='Txt3' value=''> 
                      <input type='hidden' name='Txt4' value=''> <input type='hidden' name='Txt5' value=''> 
                      <input type='hidden' name='Txt6' value=''> <input type='hidden' name='Txt7' value=''> 
                      <input type='hidden' name='Txt8' value=''> <input type='hidden' name='Txt9' value=''> 
                      <input type='hidden' name='Txt10' value=''> <input type='hidden' name='Txt11' value=''> 
                      <input type='hidden' name='Txt12' value=''> <input type='hidden' name='Txt13' value=''> 
                      <input type='hidden' name='Txt14' value=''> <input type='hidden' name='Txt15' value=''> 
                      <input type='hidden' name='Txt16' value=''> <input type='hidden' name='Bcg1'> 
                      <input type='hidden' name='Bcg2'> <input type='hidden' name='Bcg3'> 
                      <input type='hidden' name='Bcg4'> <input type='hidden' name='Bcg5'> 
                      <input type='hidden' name='Bcg6'> <input type='hidden' name='Bcg7'> 
                      <input type='hidden' name='Bcg8'> <input type='hidden' name='Bcg9'> 
                      <input type='hidden' name='Bcg10'> <input type='hidden' name='Bcg11'> 
                      <input type='hidden' name='Bcg12'> <input type='hidden' name='Bcg13'> 
                      <input type='hidden' name='Bcg14'> <input type='hidden' name='Bcg15'> 
                      <input type='hidden' name='change' value='0'> <span class="gen"> 
                      <input name="Subject" type="hidden" id="Subject2" value="d">
                      </span></td>
                  </tr>
                  <script language="javascript">
              setTimeout('LancerEditeur()',200);             
              </script>
                </table></td>
      </tr>
    </table><tr> 
    ...
    <a href="javascript:texttoobig('Act_C_Send', 1, 1)"><br/>
  <img src="images/btn_valider.gif" width="57" height="16" hspace="10" border="0"></a> 
  &nbsp; <br/>
  </form>
  </div>
</BODY>
