...
$req = MYSQL_QUERY("SELECT * FROM sondages ORDER BY id DESC");
...
function Modifier(data)
	{
	window.open("admin/modif_sondage.php?id="+data,"Modifier","toolbar=0,location=0,directories=0,menuBar=0,scrollbars=0,resizable=0,height=300,width=500")
...
{
 resultat = confirm('Voulez-vous vraiment supprimer le sondage \''+data2+'\' ?');
 if(resultat==1)
 {
  window.location='admin/suppr_sondage.php?id='+data;
 }
 else
 {
...
?>
            <tr> 
              <td class="m9" width="460"> <font size="2" face="Verdana, Arial, Helvetica, sans-serif"><? echo $disp[titre]; ?> 
                </font></td>
              <td align="center" class="m9"> 
                <?
		if ($action == 'modif')
		{
		echo "<b><a class=type1 href=\"index.php?page=modif_sondage&id=$disp[id]\">Modifier</a></b>";
		}
		else
		{
		$disp[titre] = str_replace("'","\'",$disp[titre]);
		echo "<b><a class=type1 href=\"Javascript:Supprimer('$disp[id]','$disp[titre]');\">Supprimer</a></b>";
		}
		?>
              </td>
            </tr>
