...
class post_functions extends Post {
	...
	function post_functions($class)
	{
		...
		if ( ! $this->class->forum['allow_poll'] )
		{
			$std->Error( array( LEVEL => 1, MSG => 'no_start_polls') );
		}
		...
		if ( $std->check_perms($this->class->forum['start_perms']) != TRUE )
		{
			$std->Error( array( LEVEL => 1, MSG => 'no_start_polls') );
		}
	...
	function process()
	{
		...
		{
			$this->class->obj['post_errors'] = 'poll_to_many';
		}

		if ($this->poll_count < 1)
		{
			$this->class->obj['post_errors'] = 'poll_not_enough';
		}
	...
	function add_new_poll() {
		...
		foreach ($polls as $polling)
		{
			...
			$poll_array[] = array( $count , $this->class->parser->bad_words($polling), 0 );
			$count++;
		}
		...
		$this->topic = array(
							  ...
							  'approved'         => $this->class->obj['moderate'] ? 0 : 1,
							  'pinned'           => $pinned,
							 );
		...
		$DB->do_insert( 'polls',
						 array (
								   ...
								   'starter_id'    => $ibforums->member['id'],
								   'votes'         => 0,
								   'poll_question' => $this->class->parser->bad_words($ibforums->input['pollq']),
						 )     );
		...
		if ( $this->class->obj['moderate'] == 1 OR $this->class->obj['moderate'] == 2 )
		{
			...
			$this->class->notify_new_topic_approval( $this->topic['tid'], $this->topic['title'], $this->topic['starter_name'] );
		...
		$this->class->forum_tracker($this->class->forum['id'], $this->topic['tid'], $this->topic['title'], $this->class->forum['name'] );

		//-----------------------------------------
		// Redirect them back to the topic
		//-----------------------------------------

		$std->boink_it($this->class->base_url."act=ST&f={$this->class->forum['id']}&t={$this->topic['tid']}");
	...
	function show_form()
	{
		...
		$this->class->output .= $this->class->html_start_form( array( 1 => array( 'CODE', '11' ),
															          2 => array( 'f', $this->class->forum['id']),
		...
		$start_table = $this->class->html->table_top( "{$ibforums->lang['top_txt_poll']}: {$this->class->forum['name']}");
		...
		$poll_box    = $this->class->html->poll_box( $poll, $extra);

		$end_form    = $this->class->html->EndForm( $ibforums->lang['submit_poll'] );
		...
		$this->class->output = str_replace( "<!--POLL BOX-->"    , $poll_box     , $this->class->output );
		...
		$this->class->html_add_smilie_box();

		if ( $ibforums->vars['poll_disable_noreply'] != 1 )
		{
			$this->class->output = str_replace("<!--IBF.POLL_OPTIONS-->", $this->class->html->poll_options(), $this->class->output );
		}
