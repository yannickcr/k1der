    ...
    function auto_run() {
 	...
 	function find_user_two()
 	{
		global $ibforums, $DB, $std;
 		...
 		$DB->simple_construct( array( 'select' => 'id, name',
 									  'from'   => 'members',
 									  'where'  => "LOWER(name) LIKE '".$ibforums->input['username']."%'",
 									  'limit'  => array( 0,101) ) );
		$DB->simple_exec();
 	...
 	function show_emoticons()
 	{
		global $ibforums, $DB, $std;
 		...
 		$DB->simple_construct( array( 'select' => 'typed, image', 'from' => 'emoticons', 'where' => "emo_set='".$ibforums->skin['_emodir']."'" ) );
		$DB->simple_exec();
 	...
 	function show_bbcode()
 	{
		global $ibforums, $DB, $std;
 		...
 		$bbcode = array(
						...
						11 => array('[img]', '[/img]', $ibforums->vars['board_url'].'/'.$ibforums->vars['img_url'].'/folder_post_icons/icon11.gif' ),
		...
		foreach( $bbcode as $bbc )
		{
			...
			$this->output .= $this->html->bbcode_row_header( $ibforums->lang['bbc_title']);

			$this->output .= $this->html->bbcode_row( $before, stripslashes($after) );

			$this->output .= $this->html->bbcode_row_footer();
		}
		...
		while ( $row = $DB->fetch_row() )
		{
			...
			$before = preg_replace( "#(\[".$row['bbcode_tag']."(?:[^\]]+)?\])#is", $this->html->wrap_tag("\\1"), $before );
			$before = preg_replace( "#(\[/".$row['bbcode_tag']."\])#is"          , $this->html->wrap_tag("\\1"), $before );

			$this->output .= $this->html->bbcode_row_header( $row['bbcode_title'], $row['bbcode_desc'] );

			$this->output .= $this->html->bbcode_row( $before, $t );
