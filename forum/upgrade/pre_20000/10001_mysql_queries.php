...
class upgrade_sql
{
	...
	function step_21()
	{
		...
		foreach( $SQL as $query )
		{
			...
			if ( $DB->error )
			{
				$this->error[] = $query."<br /><br />".$DB->error;
			}
			else
			{
				$this->sqlcount++;
	...
	function step_20()
	{
		...
		$SQL[] = "alter table ".SQL_PREFIX."posts change pid pid int(10) NOT NULL default '0' auto_increment;";
		...
		$SQL[] = "alter table ".SQL_PREFIX."topics change tid tid int(10) NOT NULL default '0' auto_increment;";
		...
		$SQL[] = "alter table ".SQL_PREFIX."voters change vid vid int(10) NOT NULL default '0' auto_increment;";
	...
	function step_6()
	{
		...
		$SQL[] = "alter table ".SQL_PREFIX."search_results add query_cache text;";
	...
	function step_2()
	{
	   ...
	   $SQL[] = "CREATE TABLE ".SQL_PREFIX."forums (
		...
		$oq = $DB->query("SELECT * FROM ".SQL_PREFIX."rules");

		while ( $r = $DB->fetch_row($oq) )
		{
			$nq = $DB->query("UPDATE ".SQL_PREFIX."forums SET rules_title='{$r['title']}', rules_text='{$r['body']}' WHERE id={$r['fid']}");
		}

		$DB->query("drop table if exists ".SQL_PREFIX."rules");
	}
	...
	function step_1()
	{
		...
		$oq = $DB->query("SELECT g_id, g_title FROM ".SQL_PREFIX."groups ORDER BY g_id");

		while( $r = $DB->fetch_row($oq) )
		{
			$nq = $DB->query("REPLACE INTO ".SQL_PREFIX."forum_perms SET perm_id={$r['g_id']}, perm_name='{$r['g_title']} Mask'");
			$bq = $DB->query("UPDATE ".SQL_PREFIX."groups SET g_perm_id={$r['g_id']} WHERE g_id={$r['g_id']}");
		}

		$DB->query("update ".SQL_PREFIX."messages SET read_state=1 where vid='sent'");

		$DB->query("DROP TABLE if exists ".SQL_PREFIX."attachments");
	}
