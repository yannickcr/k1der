...
+---------------------------------------------------------------------------
|
|   > IPB UPGRADE MODULE:: IPB 2.0.0 PF4 -> FINAL
...
{
	var $this_version = '20011';
	var $upgrade_from = '20010';
	...
	function upgrade_sql()
	{
		...
		if ( file_exists( THIS_PATH.'upg_'.$ibforums->current_upgrade.'/components.php' ) )
		{
			require_once( THIS_PATH.'upg_'.$ibforums->current_upgrade.'/components.php' );

			$DB->do_update( 'skin_sets', array( 'set_css'           => $CSS,
												'set_cache_css'     => $CSS,
												'set_wrapper'       => $WRAPPER,
