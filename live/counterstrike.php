...
class CounterStrike {
    ...
    function getnextstring(&$data) {
        $temp="";
        $counter=0;
        while (isset($data[$counter]) && ord($data[$counter++])!=0) $temp.=$data[$counter-1];
    ...
    // **********************************************************************
    function getServerStatus($serveraddress,$portnumber,$waittime,$rcon) {
        $cmd="\xFF\xFF\xFF\xFFchallenge rcon\x00";
		$d_timer = ereg_replace('0\.([0-9\.]*) ([0-9]*)','\\2.\\1',microtime());        
        $serverdata=$this->getServerData($cmd,$serveraddress,$portnumber,$waittime);
		$f_timer = ereg_replace('0\.([0-9\.]*) ([0-9]*)','\\2.\\1',microtime()); 
		$micro=($f_timer-$d_timer);        
        $serverdata=str_replace("\xFF\xFF\xFF\xFFchallenge rcon ","",str_replace("\n\x00","",$serverdata));
		$cmd="\xFF\xFF\xFF\xFFrcon ".$serverdata." \"".$rcon."\" setinfo";
        $serverdata=$this->getServerData($cmd,$serveraddress,$portnumber,$waittime);
		$serverdata=str_replace("\xFF\xFF\xFF\xFF\x6C","",$serverdata);
		$serverdata=explode("\n",$serverdata);
		foreach($serverdata as $i=>$var) if(ereg("status",$var)) $data=$var;
		if(!isset($data)) return false;
		while(ereg("  ",$data)) $data=str_replace("  "," ",$data);
		$status=explode(" ",$data);
		$this->m_serverstatus=$status[1];
		$this->m_serverping=$micro;
        return true;
    }    
    ...
    function getServerInfo($serveraddress,$portnumber,$waittime) {
        $cmd="\xFF\xFF\xFF\xFFA2S_INFO\x00";        
        $serverdata=$this->getServerData($cmd,$serveraddress,$portnumber,$waittime)    ;
        // Check length of returned data, if < 5 something went wrong
        if (strlen($serverdata)<5) return false;        
        // Strip OOB data                
