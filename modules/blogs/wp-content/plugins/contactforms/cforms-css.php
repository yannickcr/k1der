...
if(!empty($_POST['save_css'])){
		...
		if(is_writeable($stylefile)) {
		    ...
		    echo ' <div id="message" class="updated fade"><p><strong>'. __('Write Error! Please verify write premissions on the style file.', 'cforms') .'</strong></p></div>'."\n";
						...
						<table>
							...
							<tr height="200">
								<td colspan="3">
									...
									<input type="submit" name="LabelIDs" id="label-id" class="allbuttons addfield" value="<?php if ( get_option('cforms_labelID')=='' || get_option('cforms_labelID')=='0' ) _e('Activate Label IDs', 'cforms'); else  _e('Deactivate Label IDs', 'cforms'); ?>" />
...
?>
	<form id="editcss" method="post" action="" name="editcss">
			 ...
			 <fieldset class="cformsoptions">
				<p class="cflegend" style="margin-top:10px;"><a class="helptop" href="#top"><?php _e('top', 'cforms'); ?></a><input type="submit" name="save_css" class="allbuttons updbutton" value="<?php _e('Update Changes &raquo;', 'cforms'); ?>"/><?php _e('Basic CSS editor. Current style file: ', 'cforms'); echo '<span style="color:blue;">'.$style.'</span>' ?></p>
