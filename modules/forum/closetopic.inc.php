...
$template->setFile('centre','forum/close_topic.html');
$site->addCss('templates/'.THEME.'/forum/style.css');
$site->addToTitle(' - Forum - Fermer un sujet');
 ...
 */
if(!$membres->verifAcces('forum_'.$info['forumId'].'_del')) $site->error('<p>Vous n\'avez pas l\'autorisation de fermer ce sujet.</p>');

/**
 * ACTION ENVOI FORMULAIRE
 */
if($this->action('closeTopic')) $forum->closeTopic($_GET['topic']);

$template->setVar('message','Voulez-vous vraiment fermer le sujet "'.$info['topicTitre'].'" ?');
