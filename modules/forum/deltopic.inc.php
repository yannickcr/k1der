...
$template->setFile('centre','forum/del_topic.html');
$site->addCss('templates/'.THEME.'/forum/style.css');
$site->addToTitle(' - Forum - Supprimer un sujet');
 ...
 */
if(!$membres->verifAcces('forum_'.$info['forumId'].'_del')) $site->error('<p>Vous n\'avez pas l\'autorisation de supprimer ce sujet.</p>');

/**
 * ACTION ENVOI FORMULAIRE
 */
if($this->action('delTopic')) $forum->delTopic($_GET['topic']);

$template->setVar('message','Voulez-vous vraiment supprimer le sujet "'.$string->clean($info['topicTitre'],'htmlentities').'" ?');
