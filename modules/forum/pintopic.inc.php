...
$template->setFile('centre','forum/pin_topic.html');
$site->addCss('templates/'.THEME.'/forum/style.css');
$site->addToTitle(' - Forum - &Eacute;pingler un sujet');
 ...
 */
if(!$membres->verifAcces('forum_'.$info['forumId'].'_del')) $site->error('<p>Vous n\'avez pas l\'autorisation d\'pingler ce sujet.</p>');

/**
 * ACTION ENVOI FORMULAIRE
 */
if($this->action('pinTopic')) $forum->pinTopic($_GET['topic']);

$template->setVar('message','Voulez-vous vraiment pingler le sujet "'.$info['topicTitre'].'" ?');
