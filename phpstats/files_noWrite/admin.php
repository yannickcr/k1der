...
$error['debug_level']=0;  // Debug si sttiva da solo in caso di errore
    ...
    $norefresh_action=Array('login','logout','esclusioni','preferenze','refresh','backup','resett','downadmin','clicksadmin','optimize_tables','viewerrorlog','modify_config'); // Azioni che non hanno refresh in visualizzazione
      $cache_recn_arr=Array('main','os_browser','reso','systems','daily','weekly','monthly','calendar','compare','country','bw_lang','trend','ip');  // Azioni che usano la cache
           $page_list=Array('main','details','os_browser','reso','systems','pages','percorsi','time_pages','referer','engines','query','searched_words','bw_lang','hourly','daily','weekly','monthly','calendar','compare','country','downloads','clicks','ip','trend');
// SECURITY ISSUES
...
db_connect();
   ...
   if(mysql_num_rows($result)!=44) die("<b>ERRORE</b>: Anomalia nella tabella $option[prefix]_config, dati di configurazione in numero non corretto (non 43).");
...
if($action=='enter')
  ...
  if($pass==$option['admin_pass']){
    ...
    $is_loged_in=1;
    header("Location: $option[script_url]/admin.php?action=main");
    exit();
    }
...
if($option['use_pass']){
  $unlockedPages=explode('|',$option['unlock_pages']);
  $unlockedKey=array_search($action,$page_list);
  if($unlockedPages[$unlockedKey]==0) if($is_loged_in!=1) if($action!='wrong_pass') if($action!='send_password') $action='login';
  }
